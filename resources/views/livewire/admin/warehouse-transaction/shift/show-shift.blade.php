<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between">
        <h3 class="card-title">{{ __('msgs.all', ['name' => __('transaction.treasuries_shifts')]) }}</h3>
        @if (!$admin_has_opened_shift)
            <a href="{{ route('admin.shifts.create') }}" class="btn btn-primary">
                {{ __('msgs.create', ['name' => __('transaction.treasury_shifts')]) }}
            </a>
        @endif
    </div>
            ...
            <table id="dataTables" class="table table-vcenter table-mobile-md card-table">
                <thead>
                    <tr>
                        ...
                        <th> {{ __('transaction.date_opened') }}</th>
                        <th> {{ __('transaction.date_closed') }}</th>
                        <th> {{ __('partials.status') }}</th>
                        <th>{{ __('transaction.review_status') }}</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody class="table-tbody">
                        ...
                        <tr>
                            <td>{{ $shift->id }}
                                @if (
                                    $shift->admin_id ==
                                        auth()->guard('admin')->id())
                                    <div class="text-info">{{ __('transaction.my_shift') }}</div>
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                <span class="color-{{ $shift->is_finished ? 'green' : 'rose' }}-500 color-green-500">
                                    {{ $shift->is_finished ? __('transaction.finished') : __('transaction.not_finished') }}
                                </span>
                            </td>
                            <td>
                                @if ($shift->is_reviewed)
                                    <span class="color-green-500">{{ __('transaction.is_reviewed') }}</span>
                                @else
                                    {{ $shift->is_finished ? __('transaction.waiting_for_review') : __('transaction.opened_shift') }}
                                @endif
                                </span>
                            </td>
                            <td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7">
                                <x-blank-section :content="__('transaction.treasury_shifts')" :url="route('admin.shifts.create')" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
