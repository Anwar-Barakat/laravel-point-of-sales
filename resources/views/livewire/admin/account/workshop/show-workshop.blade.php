<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between">
        <h3 class="card-title">{{ __('msgs.all', ['name' => __('account.workshops')]) }}</h3>
        <a href="{{ route('admin.workshops.create') }}" class="btn btn-primary">
            {{ __('msgs.create', ['name' => __('account.workshop')]) }}
        </a>
    </div>
                ...
                <div class="col-12 col-md-4 col-lg-2">
                    <div class="mb-3">
                        ...
                        <select class="form-select" wire:model='order_by'>
                            ...
                            <option value="name">{{ __('account.workshop_name') }}</option>
                ...
                <tbody class="table-tbody">
                    @forelse ($workshops as $workshop)
                        <tr>
                            <td>{{ $workshop->name }}</td>
                            <td>
                                <a href="{{ route('admin.accounts.show', ['account' => $workshop->account]) }}">
                                    <span class="badge bg-info-lt">
                                        {{ $workshop->account->number }}
                                    </span>
                                </a>
                            </td>
                            <td App::getLocale()=='ar' ? style="direction: ltr" : '' class="text-center">
                                @include('layouts.balance-status', ['account' => $workshop->account])
                            </td>
                            <td>
                                <div>
                                    <button wire:click='updateStatus({{ $workshop->id }})' class="btn position-relative">
                                        {{ $workshop->is_active ? __('msgs.is_active') : __('msgs.not_active') }}
                                        <span class="badge {{ $workshop->is_active ? 'bg-green' : 'bg-red' }} badge-notification badge-blink"></span>
                                    </button>
                                </div>
                            </td>
                            ...
                            <td>
                                <span class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="{{ route('admin.workshops.edit', ['workshop' => $workshop]) }}" class="dropdown-item d-flex align-items-center gap-1">
                                        ...
                                        <a href="#" class="dropdown-item d-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#modal-danger-{{ $workshop->id }}">
                                ...
                                <x-modal-delete :id="$workshop->id" :action="route('admin.workshops.destroy', ['workshop' => $workshop])" />
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5">
                                <x-blank-section :content="__('account.workshop')" :url="route('admin.workshops.create')" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
